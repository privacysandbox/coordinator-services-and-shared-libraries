...
print_usage() {
  echo "Usage:  ./operator/terraform/util_scripts/gcp/tar_deploy/build_and_deploy_operator_tar.sh --oper_env=<environment> [--worker_image=<worker_image>] [--action=<action>] [--auto_approve=<true/false>]"
...
setup() {
  # Create temporary directory for operator
    ...
    # build tar
    bazel build //operator/terraform/gcp:operator_tar

    # extract tar
    TAR_FILE="$(bazel info bazel-bin)/operator/terraform/gcp/operator_tar.tgz"
    tar xzf "$TAR_FILE" -C "$OPER_TEMP"

    cp -r $(bazel info workspace)/operator/terraform/gcp/environments/$OPER_ENV/ "$OPER_TEMP/environments/$OPER_ENV"
...
$(bazel info workspace)/operator/terraform/gcp/util_scripts/deploy/operator_deploy.sh --oper_env_path=$OPER_ENV_PATH $PARAMETERS
