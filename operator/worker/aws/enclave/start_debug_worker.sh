...
# Script to help debug worker errors by starting a new worker process in debug
# mode (note: preventing binary attestation) and immediately attaching a
# console. To be used manually.
#
# Useful for debugging the worker when it fails to fully initialize and is in a
# flapping state (being restarted by systemd) by attaching a console immediately
# after starting the worker.
#
# Assumes at most one enclave is running on the system. Stops the worker watcher
...
echo "[Info]: Stopping the worker watcher"

set -x
# Stop worker_watcher if it's currently running/restarting the worker
systemctl stop aggregate-worker

# Check for any other lingering workers (e.g. started by this script)
...
nitro-cli run-enclave --cpu-count=2 --memory=7000 --eif-path=/opt/google/worker/enclave.eif --debug-mode
...
echo "[Info]: Run `systemctl start aggregate-worker` to re-enable the worker watcher."
