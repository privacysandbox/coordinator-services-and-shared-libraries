...
variable "environment" {
  description = "MPKHS private key endpoint service environment. Has a maximum length of 18 characters. Valid characters are lowercase alphanumeric, hyphen, or underscore."
  type        = string
}

variable "heartbeat_source_zip" {
  description = <<EOT
    Path to the zip file for privatekey synthetic heartbeat python script. The default is the zip file that is created when you
    build Privatekey prober locally using
      $ bazel build python/mpksh/aws/privatekey_synthetic:privatekey_heartbeat_pkg
    which will be create at
    `bazel-bin/python/mpkhs/aws/privatekey_synthetic/privatekey_heartbeat_pkg.zip`.
...
variable "coordinator_role" {
  description = "Coordinator role for the synthetic canary script to assume."
  type        = string
}

variable "url_to_probe" {
  description = "Coordinator MPKHS PrivateKey endpoint API url to probe."
  type        = string
}
