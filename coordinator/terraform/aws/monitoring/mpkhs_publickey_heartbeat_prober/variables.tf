...
variable "environment" {
  description = "MPKHS public key endpoint service environment. Has a maximum length of 18 characters. Valid characters are lowercase alphanumeric, hyphen, or underscore."
  type        = string
}

variable "heartbeat_source_zip" {
  description = <<EOT
    Path to the zip file for publickey synthetic heartbeat python script. The default is the zip file that is created when you
    build Publickey prober locally using
      $ bazel build python/mpksh/aws/publickey_synthetic:publickey_heartbeat_pkg
    which will be create at
    `bazel-bin/python/mpkhs/aws/publickey_synthetic/publickey_heartbeat_pkg.zip`.
...
variable "url_to_probe" {
  description = "Coordinator MPKHS PublicKey endpoint API url to probe."
  type        = string
}
