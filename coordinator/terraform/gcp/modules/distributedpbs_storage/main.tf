...
resource "google_spanner_database" "pbs_spanner_database" {
  ...
  ddl = [
    ...
    # The following DDL statement adds a new column to the budget key table.
    # This statement is not included in the google_spanner_database resource because
    # it requires uploading and creating a proto bundle, which is not supported:
    # https://github.com/hashicorp/terraform-provider-google/issues/20068
    #
    # Instead, the proto bundle and column are created outside of Terraform.
    # This comment serves as a reminder that the budget key table has a proto column
