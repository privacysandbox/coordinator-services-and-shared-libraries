...
# Network Endpoint Group to route to cloud run services in each region
resource "google_compute_region_network_endpoint_group" "public_key_cloud_run" {
  for_each = var.use_cloud_run ? google_cloud_run_v2_service.public_key_service : {}

  project               = var.project_id
  name                  = "${var.environment}-${each.value.location}-get-public-key-cr-endpoint-group"
  network_endpoint_type = "SERVERLESS"
  region                = each.value.location
  cloud_run {
    service = each.value.name
...
resource "google_compute_backend_service" "get_public_key_loadbalancer_backend" {
  ...
  dynamic "backend" {
    for_each = var.use_cloud_run ? google_compute_region_network_endpoint_group.public_key_cloud_run : google_compute_region_network_endpoint_group.get_public_key_network_endpoint_group
    content {
