...
def update_remote_state_file(project, bucket_name, file_name, file_contents):
    ...
    1. Rename the existing state file to keep it as a backup.
    2. Create a new state file with the desired state
...
def execute_deletion_statements(project, instance_id, database_id, delete_statement):
...
def process_removals(removals, project, instance_id, database_id, table_name):
    ...
    execute_deletion_statements(project, instance_id, database_id, delete_statement)


def execute_insertion_statements(project, instance_id, database_id, insert_statement):
...
def process_additions(additions, project, instance_id, database_id, table_name):
    ...
    execute_insertion_statements(project, instance_id, database_id, insert_statement)
...
def main_handler(argv):
    ...
    parser.add_argument(
        ...
        help="the remote state file name - including any prefix e.g. auth_states/prod/allowed_ids",
        type=str)
    parser.add_argument(
        "desired_state_file_path",
        help="the allowed operators file path - the local file holding the desired state",
        type=str)
    parser.add_argument(
    ...
    parser.add_argument(
        '--state_project',
        help='GCP project containing bucket containing auth file (defaults to project set by gcloud)',
        type=str)
    parser.add_argument(
        '--db_project',
        help='GCP project containing PBS Auth spanner database (defaults to project set by gcloud)',
        type=str)

    args = parser.parse_args(argv[1:])
    if not remote_state_bucket_exists(args.state_project, args.remote_state_bucket_name):
        print('Exiting...')
        sys.exit(-1)
    if not auth_spanner_resources_exist(
        args.db_project, args.auth_spanner_instance_id, args.auth_spanner_database_id, args.auth_table_name
    ):
        print('Exiting...')
        sys.exit(-1)
    ...
    current_state_file = load_remote_state_file(
        args.state_project, args.remote_state_bucket_name, args.remote_state_file_name
    )
    ...
    update_remote_state_file(
        args.state_project, args.remote_state_bucket_name, args.remote_state_file_name, desired_state_file
    )
