...
import \
    python.privacybudget.gcp.operator_onboarding.add_allowed_identities as unit
...
class AddAllowedIdentitiesTest(unittest.TestCase):
    ...
    def test_load_desired_state(self):
        ...
        self.assertEqual(
            lines[1],
            'bee@project-id2.iam.gserviceaccount.com,"[https://domain2.com, https://domain3.com]"')
        self.assertEqual(
            lines[2],
    ...
    def test_actions_to_be_taken_result_removal(self):
        ...
        self.assertEqual(
            removals[0], '- aaa@email.com,[https://domain-a.com]')
    ...
    def test_actions_to_be_taken_result_addition(self):
        ...
        # bbb should be added
        self.assertEqual(
            additions[0], '+ bbb@email.com,[https://domain-b.com]')
    ...
    def test_actions_to_be_taken_result_replacement(self):
        ...
        self.assertEqual(
            removals[0], '- ccc@email.com,[https://domain-c.com]')
    ...
    def test_sql_dml_generation_for_single_removal(self):
        ...
        with patch(
            "python.privacybudget.gcp.operator_onboarding.add_allowed_identities.execute_deletion_statements") as mock_execute_deletion_statements:
            unit.process_removals(
                removals, 'ProjectId', 'SpannerInstanceId', 'SpannerDbId', 'SpannerTableName')
    ...
    def test_sql_dml_generation_for_removals(self):
        # This is the generated diff
        removals = ['- ccc@email.com,"[https://domain-c.com, https://domain-cc.com]"',
        ...
        with patch(
            ...
            mock_execute_deletion_statements.assert_called_once_with(
                ...
                'SpannerInstanceId',
                'SpannerDbId',
                "DELETE FROM SpannerTableName WHERE AccountId IN ('ccc@email.com','ddd@email.com');")
    ...
    def test_sql_dml_generation_for_single_addition(self):
        ...
        with patch(
            "python.privacybudget.gcp.operator_onboarding.add_allowed_identities.execute_insertion_statements") as mock_execute_insertion_statements:
            unit.process_additions(
                additions, 'ProjectId', 'SpannerInstanceId', 'SpannerDbId', 'SpannerTableName')

            mock_execute_insertion_statements.assert_called_once_with(
                'ProjectId',
                'SpannerInstanceId',
                'SpannerDbId',
                "INSERT INTO SpannerTableName (AccountId,AdtechSites) VALUES ('ddd@email.com',['https://domain-d.com']);")

    def test_sql_dml_generation_for_additions(self):
        # This is the generated diff
        additions = ["+ ccc@email.com,[https://domain-c.com]",
        ...
        with patch(
            ...
            mock_execute_insertion_statements.assert_called_once_with(
                ...
                'SpannerInstanceId',
                'SpannerDbId',
                "INSERT INTO SpannerTableName (AccountId,AdtechSites) VALUES ('ccc@email.com',['https://domain-c.com']),('ddd@email.com',['https://domain-d.com', 'https://domain-dd.com']);")
    ...
    def test_top_level_execution_basic_path_when_remote_state_exists(
        ...
        mock_process_removals.assert_called_once_with(
            removals, db_project, spanner_instance_id, spanner_db_id, spanner_table_name)
        mock_process_additions.assert_called_once_with(
            additions, db_project, spanner_instance_id, spanner_db_id, spanner_table_name)
    ...
    def test_top_level_execution_should_return_early_if_too_few_arguments(
        self):
