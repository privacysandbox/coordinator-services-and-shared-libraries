...
class AuthLambdaHandlerTest(unittest.TestCase):
  ...
  def test_unauthorized_if_missing_request_context(self):
    ...
    self.assertEqual(auth_lambda_handler.authorization_failure_stage, 'context')
  ...
  def test_unauthorized_if_missing_user_arn(self):
    ...
    self.assertEqual(ret['statusCode'], 403)
    self.assertEqual(auth_lambda_handler.authorization_failure_stage, 'userArn')
  ...
  def test_unauthorized_if_missing_identity_header(self):
    event = {'requestContext': {'identity': {'userArn': 'arn'}}, 'headers': {}}
  ...
  def test_should_transform_assume_role_arn(self):
    ...
    event = {
        'requestContext': {'identity': {'userArn': assume_role_arn}},
    ...
    dynamodb_resource_mock = boto3.DynamoDbResourceMock(dynamodb_table_mock)
    ...
    # The transformed ARN
    self.assertEqual(
        dynamo_key_used['adtech_identity'],
  ...
  def test_should_parse_public_suffix(self):
    ...
    event = {
        ...
        'headers': {
            'x-gscp-claimed-identity': reported_origin,
  ...
  def test_should_parse_public_suffix_with_port(self):
    ...
    user_arn = 'arn:aws:iam::123456789012:role/demo'
    reported_origin = (
        'extra-prefix.top-level-private-suffix.kimitsu.chiba.jp:8888'
    )
  ...
  def test_authorized_if_no_protocol_reported_origin_belongs_to_allowlisted_site(
    ...
    # Item the DB returns
    db_item_return = {
        'Item': {'adtech_sites': {'https://my-top-level-site.com'}}
    }
