...
import com.google.scp.protocol.avro.AvroReportWriter;
import com.google.scp.protocol.avro.AvroReportWriterFactory;
    ...
    avroReportWriter.writeRecords(
        ...
        ImmutableList.of(
            AvroReportRecord.create(
                ByteSource.wrap(new byte[] {0x01}), UUID.fromString(KEY_ID).toString())));
  }

  @Test
  public void of_localNioPath() throws RecordReadException {
    DataLocation dataLocation =
        BlobStorageClient.getDataLocation("", avroFile.getFileName().toString());
    DataLocation avro = DataLocation.ofLocalNioPath(avroFile);
    // does not throw exception.
    assertThat(avroRecordReaderFactory.of(avro)).isNotNull();
  }

  @Test
  public void of_throwsExceptionForNonExistentLocalNioFilePath() {
    DataLocation dataLocation = BlobStorageClient.getDataLocation("fakeDirectory", "fakeFile");
