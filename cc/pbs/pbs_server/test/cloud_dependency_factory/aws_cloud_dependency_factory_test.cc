 ...
 protected:
  AwsCloudDependencyFactoryTest()
      : async_executor1_(make_shared<core::AsyncExecutor>(2, 10000, true)),
        ...
        aws_factory_(mock_config_provider_) {}
  ...
  shared_ptr<HttpClientInterface> mock_http_client_;
  shared_ptr<AuthTokenProviderInterface> mock_auth_token_;
  ...
  AwsDependencyFactory aws_factory_;
};

TEST_F(AwsCloudDependencyFactoryTest,
       ConstructAuthorizationTokenProviderCache) {
  auto cache = aws_factory_.ConstructAuthorizationTokenProviderCache(
      async_executor1_, async_executor2_, mock_http_client_);
  EXPECT_THAT(cache->Init(), ResultIs(SuccessExecutionResult()));
  // TODO: Enable this. Run() is crashing in the test deep within the AWS STS
  // function call stack, when run within the test case. Debug further
  // EXPECT_THAT(cache->Run(),
  // ResultIs(SuccessExecutionResult())); EXPECT_THAT(cache->Stop(),
  // ResultIs(SuccessExecutionResult()));
}

TEST_F(AwsCloudDependencyFactoryTest, ConstructAuthorizationProxyClient) {
  auto proxy = aws_factory_.ConstructAuthorizationProxyClient(
...
TEST_F(AwsCloudDependencyFactoryTest, ConstructBlobStorageClient) {
  auto blob_client = aws_factory_.ConstructBlobStorageClient(async_executor1_,
                                                             async_executor2_);
...
TEST_F(AwsCloudDependencyFactoryTest, ConstructNoSQLDatabaseClient) {
  auto no_sql_client = aws_factory_.ConstructNoSQLDatabaseClient(
...
TEST_F(AwsCloudDependencyFactoryTest, ConstructInstanceMetadataClient) {
  auto metadata_client = aws_factory_.ConstructInstanceMetadataClient(
      mock_http_client_, mock_http_client_, async_executor1_, async_executor2_,
      mock_auth_token_);
...
TEST_F(AwsCloudDependencyFactoryTest, ConstructMetricClient) {
  auto metric_client = aws_factory_.ConstructMetricClient(
...
TEST_F(AwsCloudDependencyFactoryTest, ConstructRemoteCoordinatorPBSClient) {
  auto client = aws_factory_.ConstructRemoteCoordinatorPBSClient(
