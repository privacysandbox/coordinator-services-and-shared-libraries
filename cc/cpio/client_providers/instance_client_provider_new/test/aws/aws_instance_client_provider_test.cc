...
#include "cpio/client_providers/instance_client_provider_new/mock/aws/mock_ec2_client.h"
...
class AwsInstanceClientProviderNewTest : public TestWithParam<string> {
 protected:
  AwsInstanceClientProviderNewTest()
      : http_client_(make_shared<MockCurlClient>()),
        ...
        instance_provider_(make_unique<AwsInstanceClientProviderNew>(
  ...
  unique_ptr<AwsInstanceClientProviderNew> instance_provider_;

  SDKOptions options_;
};

TEST_F(AwsInstanceClientProviderNewTest,
       GetCurrentInstanceResourceNameSuccess) {
...
TEST_F(AwsInstanceClientProviderNewTest, GetInstanceDetailsByResourceName) {
  EXPECT_CALL(*ec2_client_,
              DescribeInstancesAsync(InstanceIdMatches(kInstanceIdMock), _, _))
      .WillOnce([&](auto, auto callback, auto) {
        DescribeInstancesRequest request;
...
TEST_F(AwsInstanceClientProviderNewTest, GetTagsByResourceNameSucceed) {
  EXPECT_CALL(*ec2_client_,
...
TEST_F(AwsInstanceClientProviderNewTest, GetTagsByResourceNameEC2ClientCached) {
  EXPECT_CALL(*ec2_client_,
              DescribeTagsAsync(FilterResourceIdMatches(kInstanceIdMock), _, _))
      .Times(4)
