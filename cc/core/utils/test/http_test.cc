...
TEST(HttpTest, NullHeadersExtractRequestClaimedIdentity) {
  ...
  ExecutionResultOr<absl::string_view> extraction_result =
...
TEST(HttpTest, ValidUserAgentHeader) {
  ...
  std::string user_agent = "aggregation-service/2.5.0";

  request_headers.insert({std::string(kUserAgentHeader), user_agent});

  ExecutionResultOr<absl::string_view> extraction_result =
      utils::ExtractUserAgent(request_headers);
...
TEST(HttpTest, InvalidUserAgentFormat) {
  ...
  request_headers.insert({std::string(kUserAgentHeader), invalid_user_agent});
...
TEST(HttpTest, EmptyUserAgentString) {
  core::HttpHeaders request_headers;
  request_headers.insert(
      {std::string(kUserAgentHeader), ""});  // Empty User-Agent.
