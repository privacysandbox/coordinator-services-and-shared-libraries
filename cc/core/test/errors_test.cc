...
TEST(ERRORS, NewHttpStatusCodeUsedWhenAvailableAndEnabled) {
  REGISTER_COMPONENT_CODE(COMPONENT_NAME, 0x7FFA)
  ...
  HttpStatusCode http_status_code =
      GetErrorHttpStatusCode(COMPONENT_NAME_ERROR, true);
  EXPECT_EQ(http_status_code, HttpStatusCode::INTERNAL_SERVER_ERROR);
}

TEST(ERRORS, DefaultHttpStatusCodeUsedWhenNewHttpStatusCodeNotAvailable) {
  REGISTER_COMPONENT_CODE(COMPONENT_NAME, 0x7FFB)
  ...
  HttpStatusCode http_status_code =
      GetErrorHttpStatusCode(COMPONENT_NAME_ERROR, true);
  EXPECT_EQ(http_status_code, HttpStatusCode::BAD_REQUEST);
}

TEST(ERRORS, NewHttpStatusCodeNotUsedWhenNotEnabled) {
  REGISTER_COMPONENT_CODE(COMPONENT_NAME, 0x7FFC)
  ...
  HttpStatusCode http_status_code =
      GetErrorHttpStatusCode(COMPONENT_NAME_ERROR, false);
  EXPECT_EQ(http_status_code, HttpStatusCode::BAD_REQUEST);
}

TEST(ERRORS, NewHttpStatusCodeNotUsedWhenNotEnabledAndNotPresent) {
  REGISTER_COMPONENT_CODE(COMPONENT_NAME, 0x7FFD)

  DEFINE_ERROR_CODE_1(COMPONENT_NAME_ERROR, COMPONENT_NAME, 0x0001,
