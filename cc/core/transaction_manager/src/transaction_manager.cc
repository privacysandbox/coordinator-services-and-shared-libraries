...
static constexpr size_t kTransactionManagerMetricTimeDurationMs = 1000;
static constexpr char kTransactionManager[] = "TransactionManager";
static constexpr char kActiveTransaction[] = "ActiveTransaction";
static constexpr char kReceivedTransaction[] = "ReceivedTransaction";
static constexpr char kFinishedTransaction[] = "FinishedTransaction";
...
ExecutionResult TransactionManager::RegisterAggregateMetric(
  ...
  MetricLabelsBase label_base(kTransactionManager, name);
  metric_info->labels =
      make_shared<MetricLabels>(label_base.GetMetricLabelsBase());
  vector<string> event_list = {kReceivedTransaction, kFinishedTransaction};
  metrics_instance =
      make_shared<AggregateMetric>(async_executor_, metric_client_, metric_info,
                                   kTransactionManagerMetricTimeDurationMs,
...
ExecutionResult TransactionManager::Init() noexcept {
  ...
  RegisterAggregateMetric(active_transactions_metric_, kActiveTransaction);
...
ExecutionResult TransactionManager::Execute(
  ...
  function<void()> task = [this, transaction_context]() mutable {
    ...
    active_transactions_metric_->Increment(kReceivedTransaction);
    ...
    transaction_engine_context.callback =
        [this, transaction_context](
          ...
          active_transactions_count_--;
          active_transactions_metric_->Increment(kFinishedTransaction);
        };
  ...
  return async_executor_->Schedule(task, AsyncPriority::Normal);
}
...
ExecutionResult TransactionManager::GetTransactionStatus(
  ...
  return transaction_engine_->GetTransactionStatus(
      get_transaction_status_context);
}

ExecutionResult TransactionManager::GetStatus(
  ...
  // This assumes that GetStatus always wants to know about pending transaction
  // count, but can be extended to other fields as needed in future.
