...
resource "aws_iam_role" "codebuild_role" {
  name = "coord_svcs_codebuild_role"
...
resource "aws_codebuild_project" "bazel_build_container" {
  ...
  description   = "Build container for coordinator services build"
  build_timeout = "10"
  ...
  source {
    type            = "CODECOMMIT"
    location        = var.coordinator_services_git_repo
    git_clone_depth = 1

    buildspec = "build-scripts/aws/build-container/buildspec.yml"
  }

  source_version = var.coordinator_services_git_repo_branch == "" ? "v${local.release_version}" : var.coordinator_services_git_repo_branch
}


# Setup build project to build  service AMI and jar artifacts
resource "aws_codebuild_project" "coordinator-services-artifacts-build" {
  name           = "coordinator-services-artifacts-build"
  description    = "Build coordinator services keygen AMI and deployment archive"
  build_timeout  = "60"
  queued_timeout = "60"
  ...
  environment {
    ...
    environment_variable {
      name  = "ARCHIVE_PUBLISH_BUCKET"
      value = var.build_artifacts_output_bucket
    }

    environment_variable {
      name  = "ARCHIVE_PUBLISH_BUCKET_PATH"
      value = "coordinator-services-build-artifacts"
    }
  }
